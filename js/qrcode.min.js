/*! Bug-free Standalone QRCode Engine for Nota Charge App - Level M Enhanced */
(function (root, factory) {
    ...
    mod: function (e) {
      ...
      while (num.length >= e.getLength()) {
        ...
        for (var i = 0; i < e.getLength(); i++) {
          var ve = e.get(i);
          if (ve !== 0) {
            num[i] ^= QRMath.gexp(QRMath.glog(ve) + ratio);
          }
        }
        num.shift();
      }
  ...
  for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 5] ^ QRMath.EXP_TABLE[i - 8];
  ...
  var QRUtil = {
    ...
    opts = opts || {};
    var targetWidth = opts.width || 220;
    var margin = (opts.margin !== undefined) ? opts.margin : 1;

    try {
      var model = new QRCodeModel(0, 1); // Error Correction Level M = 1
      model.addData(text);
      model.make();
      ...
      // PERBAIKAN: Hitung skala bilangan bulat murni untuk tiap kotak (modul)
      // Skala minimal adalah 1 piksel per modul
      var scale = Math.max(1, Math.floor(targetWidth / totalModules)); 

      // Ukuran asli canvas disesuaikan dengan perkalian bulat (mencegah pixel meleyot/pecah)
      var actualSize = totalModules * scale;

      canvas.width = actualSize;
      canvas.height = actualSize;

      // PERBAIKAN UI: Pertahankan ukuran visual di web menggunakan CSS agar layout tidak mengecil/membesar
      canvas.style.width = targetWidth + 'px';
      canvas.style.height = targetWidth + 'px';
      // Pastikan rendering tajam di browser
      canvas.style.imageRendering = 'pixelated';

      var ctx = canvas.getContext('2d');
      // Matikan antialiasing pada canvas
      ...
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (model.isDark(r, c)) {
            var x = (margin + c) * scale;
            var y = (margin + r) * scale;
            // Render kotak presisi tinggi dengan angka bulat
  ...
  function QRCodeConstructor(element, options) {
    ...
    toCanvas(canvas, text, { width: width, margin: 1 });
  }

  QRCodeConstructor.toCanvas = toCanvas;
