/*! Robust, non-recursive QRCode Library supporting toCanvas and DOM element rendering */
(function (root, factory) {
  ...
  // QR Code generator core algorithm (Iterative Non-Recursive)
  ...
  // Iterative non-recursive QRPolynomial modulo
  ...
  // Helper method: toCanvas(canvas, text, opts, callback)
  function toCanvas(canvas, text, opts, callback) {
    ...
    try {
      var model = new QRCodeModel(0, 0); // Error Correction Level M = 0
      model.addData(text);
      model.make();
      ...
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (model.isDark(r, c)) {
            var x = margin + c * tile;
            var y = margin + r * tile;
            ctx.fillRect(Math.floor(x), Math.floor(y), Math.ceil(tile), Math.ceil(tile));
          }
        }
      }
  ...
  // Constructor class for new QRCode(element, options)
